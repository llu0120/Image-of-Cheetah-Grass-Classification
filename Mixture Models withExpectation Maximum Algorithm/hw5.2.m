...
%plug in the parameters of 5 FG & BG mixture models to mvnpdf ---> BDR
error_vector=[];
countc = 1;
for c=class
for n1 = 1
    ...
    for n2 = 1
        ...
        for dim = dimension
            dim
            count = 1;
            PI_BG = priorBG_cell{countc};
            MU_BG = muBG_cell{countc};
            SIGMA_BG = sigmaBG_cell{countc};
            PI_FG = priorFG_cell{countc};
            MU_FG = muFG_cell{countc};
            SIGMA_FG = sigmaFG_cell{countc};
            for i=1:255
                for j=1:270
                    Pxy_xgrass = 0;
                    Pxy_xcheeta = 0; 
                        for C = 1:c
                            SIGMABG=SIGMA_BG{C};
                            G_grass = mvnpdf((feature_map(count,1:dim)), MU_BG(C,1:dim), SIGMABG(1:dim,1:dim));
                            Pxy_xgrass = Pxy_xgrass + PI_BG(1,C)*G_grass;
                            SIGMAFG=SIGMA_FG{C};
                            G_cheeta = mvnpdf((feature_map(count,1:dim)), MU_FG(C,1:dim), SIGMAFG(1:dim,1:dim));
...
%% plot POE and Dimemsion with different parameters
